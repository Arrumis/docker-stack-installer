...
次の 1 本は、最小パッケージの導入、リポジトリの取得、設定ファイルの作成、必要値の質問、インストール、起動確認までまとめて行います。
...
`--owner Arrumis` は、GitHub の `Arrumis` 配下にあるリポジトリ群を取得するという意味です。
このリポジトリ群をそのまま使うなら変更しません。
分岐して別アカウントで管理する場合だけ置き換えます。
...
通常の取得先は `~/docker-stack/docker-stack-installer` です。
...
設定編集後に再実行する場合は、この 1 本でインストール、起動、起動後確認、設定一覧の再出力まで進めます。
...
インストールログは `logs/install-YYYYmmdd-HHMMSS.log` に残ります。

確認できる内容:

- `sample.com` / `ttrss.*` / `munin.*` のリバースプロキシ経由応答
- `tategaki.*` / `syncthing.*` / `openvpn.*` / `epgstation.*` のリバースプロキシ経由応答
- `traefik.*` の管理画面応答
- Syncthing / OpenVPN / Tategaki のローカル応答
- Mirakurun API と EPGStation のローカル応答
