...
改修ブランチを main と同じ bootstrap 導線で使う場合は、対象PCで次の形で実行します。
`sample.com` と `/mnt/data/...` は実環境に合わせて置き換えます。

```bash
bash -lc 'set -e; sudo apt-get update; sudo apt-get install -y ca-certificates curl; curl -fsSL https://raw.githubusercontent.com/Arrumis/docker-stack-installer/refactor/align-runtime-containers/scripts/bootstrap-clean-ubuntu.sh | bash -s -- --owner Arrumis --branch refactor/align-runtime-containers --domain sample.com --root-host sample.com --email admin@sample.com --public-scheme https --host-data-root /mnt/data/var/docker --recorded-root /mnt/data/recorded'
```

すでに repo を clone 済みの場合は、次の汎用入口でも同じ設定を作れます。

```bash
cd ~/docker-stack/docker-stack-installer
./scripts/install-existing-data-stack.sh \
  --domain sample.com \
  --root-host sample.com \
  --email admin@sample.com \
  --host-data-root /mnt/data/var/docker \
  --recorded-root /mnt/data/recorded
```

まず env と repo だけ作って、起動前に内容を確認したい場合:
