...
端末やチャットで長い 1 行が途中改行される場合は、次のコピー用コマンドを使います。
行末の `\` が「次の行へ続く」という意味なので、この形なら複数行で貼っても `curl` の URL が欠けません。

```bash
bash -lc 'set -e; \
sudo apt-get update; \
sudo apt-get install -y ca-certificates curl; \
curl -fsSL https://raw.githubusercontent.com/Arrumis/docker-stack-installer/refactor/align-runtime-containers/scripts/bootstrap-clean-ubuntu.sh \
  | bash -s -- --owner Arrumis --guided'
```
